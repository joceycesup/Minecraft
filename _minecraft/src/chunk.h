...
public:
	...
	void renderOpaque (void) {
		...
		//On bind le buffer
		glBindBuffer (GL_ARRAY_BUFFER, _BufWorld);
		NYRenderer::checkGlError ("glBindBuffer chunk opaque");
	...
	void renderTransparent (bool underwater) {
		glEnable (GL_COLOR_MATERIAL);
		glEnable (GL_LIGHTING);
		glEnable (GL_TEXTURE_2D);//*
		glEnable (GL_CULL_FACE);/*/
		...
		glDisable (GL_CULL_FACE);//*/
		glEnable (GL_BLEND);// glEnable (GL_BLEND); glBlendFunc (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
		...
		//On bind le buffer
		glBindBuffer (GL_ARRAY_BUFFER, _BufWorldTransparent);
		NYRenderer::checkGlError ("glBindBuffer chunk transparent");
		...
		//On place les pointeurs sur les datas, aux bons offsets
		glVertexPointer (3, GL_FLOAT, 0, (void*)(0));
		glColorPointer (3, GL_FLOAT, 0, (void*)(_NbVerticesTransparent*SIZE_VERTICE));
		glNormalPointer (GL_FLOAT, 0, (void*)(_NbVerticesTransparent*SIZE_VERTICE + _NbVerticesTransparent*SIZE_COLOR));
		glTexCoordPointer (2, GL_FLOAT, 0, (void*)(_NbVerticesTransparent*SIZE_VERTICE + _NbVerticesTransparent*SIZE_COLOR + _NbVerticesTransparent*SIZE_NORMAL));

		//On demande le dessin
		glDrawArrays (GL_TRIANGLES, 0, _NbVerticesTransparent);
...
private:
	void cubeToVBO (int x, int y, int z) {
		bool opaque = !_Cubes[x][y][z].isTransparent ();

		//On utilise les buffers temporaires pour préparer nos datas
		float * ptVert = opaque ? _WorldVert : _WorldVertTransparent;
		float * ptCols = opaque ? _WorldCols : _WorldColsTransparent;
		float * ptNorm = opaque ? _WorldNorm : _WorldNormTransparent;
		float * ptUV = opaque ? _WorldUV : _WorldUVTransparent;

		int offset = opaque ? _NbVertices : _NbVerticesTransparent;
		...
		if (_Cubes[x][y][z].checkFace (FACE_X_PLUS)) {
			offset = opaque ? _NbVertices * 3 : _NbVerticesTransparent * 3;
			ptVert[offset + 0] = xPos + NYCube::CUBE_SIZE;
			ptVert[offset + 1] = yPos;
			ptVert[offset + 2] = zPos;

			ptCols[offset + 0] = color[0];
			ptCols[offset + 1] = color[1];
			ptCols[offset + 2] = color[2];

			ptNorm[offset + 0] = 1;
			ptNorm[offset + 1] = 0;
			ptNorm[offset + 2] = 0;

			offset = opaque ? _NbVertices * 2 : _NbVerticesTransparent * 2;
			ptUV[offset + 0] = NYCube::_CubeUV[cubeTexOffset + 0];
			ptUV[offset + 1] = NYCube::_CubeUV[cubeTexOffset + 1];

			if (opaque) _NbVertices++; else _NbVerticesTransparent++;

			offset = opaque ? _NbVertices * 3 : _NbVerticesTransparent * 3;
			ptVert[offset + 0] = xPos + NYCube::CUBE_SIZE;
			ptVert[offset + 1] = yPos + NYCube::CUBE_SIZE;
			ptVert[offset + 2] = zPos;
			...
			offset = opaque ? _NbVertices * 2 : _NbVerticesTransparent * 2;
			ptUV[offset + 0] = NYCube::_CubeUV[cubeTexOffset + 6];
			ptUV[offset + 1] = NYCube::_CubeUV[cubeTexOffset + 7];
			...
			ptVert[offset + 0] = xPos + NYCube::CUBE_SIZE;
			ptVert[offset + 1] = yPos + NYCube::CUBE_SIZE;
			ptVert[offset + 2] = zPos + NYCube::CUBE_SIZE;
			...
			offset = opaque ? _NbVertices * 2 : _NbVerticesTransparent * 2;
			ptUV[offset + 0] = NYCube::_CubeUV[cubeTexOffset + 4];
			ptUV[offset + 1] = NYCube::_CubeUV[cubeTexOffset + 5];
			...
			offset = opaque ? _NbVertices * 2 : _NbVerticesTransparent * 2;
			ptUV[offset + 0] = NYCube::_CubeUV[cubeTexOffset + 2];
			ptUV[offset + 1] = NYCube::_CubeUV[cubeTexOffset + 3];
